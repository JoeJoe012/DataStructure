...
/**
 * 将指定元素插入到此链表中的指定位置
 *   ·单链表中的节点都有一个指向后继节点的指针, 因此插入操作的关键是找到指定位置的前驱节点(也就是上一个节点)
 * @param head 操作链表的头节点
...
bool insertList(Node head, E element, int index) {
    ...
    while (--index) {    //while(--index){}条件表达式循环index-1次, 寻找目标位置的前驱节点
        head = head->next;
        if (head == NULL)    //若寻找过程中没有后续的节点, 则说明index超出可插入的范围
            return false; 
    }
    Node newNode = malloc(sizeof(struct ListNode));
    if (newNode == NULL) return false;
    newNode->element = element;
    newNode->next = head->next;    //到这步, head指向目标位置的前驱节点
    head->next = newNode;
    return true;
}
/**
 * 删除此链表中指定位置的元素
 *   ·单链表删除操作的关键是找到目标位置的前驱节点
 * @param head 操作链表的头节点
...
bool deleteList(Node head, int index) {
    ...
    if (head->next == NULL) {    //待删除节点为NULL则直接返回false
        return false;
    }
